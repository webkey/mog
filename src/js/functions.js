...
(function ($) {
	...
	MainNavigation.prototype.toggleMenu = function () {
		...
		$('body').on('click', ''+self.options.btnMenu+'', function (e) {
			e.preventDefault();
	...
	MainNavigation.prototype.addShadowOnScroll = function () {
		var self = this;

		$('.extra-popup__content').on('scroll', function () {
...
function buyPopup() {
	if ($('.buy-popup').length) {
		...
		new MainNavigation({
			navContainer: '.buy-popup',
			navMenu: '.main-menu__list',
			btnMenu: 'a.btn-bay',
			btnMenuClose: '.js-btn-menu-close',
			navMenuItem: '.main-menu__box',
			overlayAppendTo: 'body',
			closeOnResize: false, // mediaWidth: 1280,
			animationSpeed: 300,
			overlayAlpha: 0.35
		});
...
function priemPopup() {
    if ($('.priem-popup').length) {

        new MainNavigation({
            navContainer: '.priem-popup',
            navMenu: '.main-menu__list',
            btnMenu: 'a.priem-form',
            btnMenuClose: '.js-btn-menu-close',
            navMenuItem: '.main-menu__box',
            overlayAppendTo: 'body',
            closeOnResize: false, // mediaWidth: 1280,
 ...
 * enter popup
 * */
function branchesSchemePopup(){
	if($('.branches-scheme-popup').length){

		new MainNavigation({
			navContainer: '.branches-scheme-popup',
			navMenu: '.main-menu__list',
			btnMenu: '.map-link a',
			btnMenuClose: '.js-btn-menu-close',
			navMenuItem: '.main-menu__box',
...
function branchesMapPopup(){
	...
	if ($popup.length) {
		...
		function closePopup() {
			$('.branches-map-js svg a').removeClass(classActive);
